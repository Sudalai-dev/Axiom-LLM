"""Usage metering — writes real per-user daily aggregates to UsageMetric.
...
:func:`record_solution_usage`, which upserts the user's row for the current
...
async def record_solution_usage(req_ctx: RequestContext, output) -> None:
    """Upserts today's usage aggregate for the user with this call's usage."""
